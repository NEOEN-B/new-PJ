...
LOCK = RLock()
LAST_ERROR = ""
RSS_TIMEOUT_SECONDS = float(os.getenv("RSS_TIMEOUT_SECONDS", "8"))
...
def fetch_feed_with_timeout(source: str) -> feedparser.FeedParserDict:
    req = Request(
        source,
        headers={"User-Agent": "ai-digest-bot/1.0 (+https://localhost)"},
    )
...
def fetch_latest_ai_articles(limit: int = 50) -> Tuple[List[Dict[str, str]], List[str]]:
    articles: List[Dict[str, str]] = []
    failed_sources: List[str] = []

    for source in RSS_SOURCES:
        try:
            feed = fetch_feed_with_timeout(source)
    ...
    return final_items[:limit], failed_sources
...
def build_daily_digest(force_refresh: bool = False) -> List[Dict[str, str]]:
    ...
    try:
        ...
        candidates, failed_sources = fetch_latest_ai_articles(limit=50)
        ...
        if failed_sources:
            failed_text = "、".join(failed_sources)
            set_last_error(f"部分订阅源拉取失败，已自动跳过：{failed_text}")
        else:
            set_last_error("")
        return result
    except Exception:
